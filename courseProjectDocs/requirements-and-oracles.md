...
|----------------|-------------------------|---------------------------------|
| FR-1, FR-2 | The system shall provide symmetric encryption and key generation. | **Oracle 1:** If a user generates a key using Fernet, uses it to encrypt the string "secret data", and decrypts the ciphertext with the same key, the output should exactly match "secret data". |
